...
//            Log.w("ceshi","");
            ...
            {
//                Log.w("ceshi","asdb:"+((COSObject)token).getObjectNumber()+"-"+((COSObject)token).getGenerationNumber());
                arguments.add(((COSObject) token).getObject());
            }
            else if (token instanceof Operator)
            {
//                Log.w("Operator_ceshi","Operator:"+((Operator) token).getName());
                processOperator((Operator) token, arguments);
                arguments = new ArrayList<COSBase>();
            }
            else
            {
//                Log.w("ceshi","bbb:"+token.getClass().getSimpleName());
//                if (token instanceof COSNumber) {
//                    Log.w("ceshi","token:"+((COSNumber)token).floatValue());
//                }
...
//        Log.w("ceshi"," test++==="+test);
    ...
    private void clipToRect(PDRectangle rectangle)
    {
        if (rectangle != null)
        {
            ...
            getGraphicsState().intersectClippingPath(clip,scale);
        }
    }
